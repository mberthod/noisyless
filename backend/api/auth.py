...
async def send_verification_email(email: str, token: str, background_tasks: BackgroundTasks):
    ...
    verification_url = f"https://platform.noisyless.com/auth/verify?email={email}&token={token}"
...
async def contact_form(request: ContactRequest, background_tasks: BackgroundTasks):
    """Send contact form email to NOISYLESS team"""
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <h2 style="color: #4f6ef7;">Nouvelle demande de pilote</h2>
            ...
            <table style="width: 100%; margin-top: 20px;">
                ...
                {f'<tr><td style="padding: 8px 0; font-weight: 600;">Société :</td><td style="padding: 8px 0;">{request.company}</td></tr>' if request.company else ''}
                <tr>
                    <td style="padding: 8px 0; font-weight: 600;">Message :</td>
                    <td style="padding: 8px 0;">{request.message}</td>
                </tr>
            </table>
    ...
    send_email("contact@noisyless.com", f"Nouvelle demande pilote - {request.name}", html_content)
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <h2 style="color: #4f6ef7;">Merci pour votre demande !</h2>
            <p>Nous avons bien reçu votre demande de pilote gratuit et vous répondrons sous 24h ouvrées.</p>
            
            <p style="margin-top: 20px;">En attendant, vous pouvez :</p>
            <ul style="line-height: 2;">
                <li>Consulter notre <a href="https://noisyless.com" style="color: #4f6ef7;">site web</a></li>
                <li>Découvrir notre <a href="https://platform.noisyless.com" style="color: #4f6ef7;">plateforme</a></li>
            </ul>
            
            <hr style="border: none; border-top: 1px solid #eee; margin: 2rem 0;">
            
            <p style="color: #999; font-size: 0.85rem;">
                L'équipe NOISYLESS<br>
    ...
    send_email(request.email, "Merci pour votre demande NOISYLESS", confirm_html)
    
    return {"status": "ok", "message": "Demande envoyée avec succès"}
